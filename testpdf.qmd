...
names(df) <- c("År", "Land", "Mål", "Målenhet", "Produktivitet", "baseår") # Endrer kolonnenavnene veldig enkelt med denne koden
```
...
# Husk å legge til figurnummer i toppen av kodechuncken
...
df_vekst %>%
  ...
  geom_col(position = "dodge") + # Bruker geom_col for kolonner
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  scale_x_continuous(breaks = seq(min(df_vekst$År), max(df_vekst$År), by = 2)) +
  labs(
  ...
  facet_wrap(~Land, ncol = 2)#, scales = "free_y")
# Husk kilde til figuren
# her sier derek at dte kan være lurt å dele opp slik at det blir en figur til hvert land, vi kan sikkert ta bort Tyskland?
```
...
# unique(df_09170$målenhet) # Henter ut navnene
...
names(df_09174) <- c("næring", "målenhet", "år", "verdi") #Endrer kolonnenavnene veldig enkelt med denne koden

#unique(df_09174$målenhet) # Henter ut navnene
...
# Beregn andelen av total produksjon
df <- df %>%
  mutate(vekt = bruttoprodukt_mill_kr / total_bruttoprodukt_mill)  # Andel av total produksjon

# Beregn bidrag til produktivitetsutvikling
df <- df %>%
  mutate(vektet_bidrag = vekt * vekst)


# en figur for prosentvis vekst, og en figur til prosentvis vekst som er vektet. Er det noen forskjeller der?
```
...
bidrag_næringer <- df %>%
  ...
  summarise(bidrag_per_næring = sum(vektet_bidrag, na.rm = TRUE)) %>%  # Summer produktivitetsvekst for hver næring
  arrange(desc(bidrag_per_næring)) # Sortér fra størst til minst vekst

bidrag_næringer <- bidrag_næringer[!(bidrag_næringer$næring %in% c("Utvinning av råolje og naturgass, inkl. tjenester", "Utenriks sjøfart", "Rørtransport")), ]

topp4_næringer <- bidrag_næringer %>%
  slice_head(n = 4) %>%     # Ta de 10 øverste radene
  pull(næring)
#print(topp4_næringer)
```

```{r warning=FALSE, message=FALSE}
#| echo: false

# Denne koden har chatgpt hjulpet med for å representere funnene mine oversiktelig

topp4_tabell <- data.frame(Næring = topp4_næringer)
...
bunn4_næringer <- bidrag_næringer %>%
  slice_tail(n = 4) %>%  # Hent de 10 siste radene
  pull(næring)
#print(bunn4_næringer)
```
...
knitr::kable(
  bunn4_tabell,
  col.names = "Næring",
  caption = "De fire næringene med lavest bidrag til produktivitet."
) %>%
  kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "bordered"),
    full_width = FALSE
...
For å få en bedre forståelse av hvordan de fire største og de fire minste bidragene til produktivitetsutviklingen i Norge opptrer, visualiseres de i to figurer.

```{r warning=FALSE, message=FALSE}
#| echo: false
#| label: fig-topp4
#| fig-cap:
#|   - "Figuren viser det vektede bidraget til næringene og veksten disse næringen har hatt i peridoen 2015-2023"
...
combined_plot <- plot_1 + plot_2 +
  ...
  labs(caption = "Kilde: Statistisk sentralbyrå (2024)")
...
#| label: fig-bunn4
#| fig-cap:
#|   - "Max nytte av konsum og fritid"

# Fikk hjelo av chatgpt med denne koden:
combined_plot_2 <- plot_12 + plot_22 +
  plot_layout(guides = 'collect') & # deler legende
  theme(legend.position = "bottom") + # legger legenden nederst
  labs(caption = "Kilde: Statistisk sentralbyrå (2024)")
...
befolkning$region <- factor(befolkning$region, levels = c("Oslo", "Hammerfest - Hámmerfeasta", "Bergen", "Rana - Raane", "Stavanger", "Sogndal")) # Sorterer de slik at det blir lettere å lese av data i plottet
...
# df_utdanning <- pivot_wider(df_utdanning, names_from = utdanningsnivå, values_from = value)
...
df_utdanning <- df_utdanning %>%
  ...
  mutate(vekst = ((value-lag(value))/lag(value))*100)
# prøv å se om prosentvis veskt i andlen av befolkningen gi bedre tall for plottet
```

```{r warning=FALSE, message=FALSE}
#| echo: false
#| label: fig-utdanning
#| fig-cap: "Utviklingen av høyere utdanning i Norge, mål for personer over 16 år i prosentandel av befolkningen. Dataen for den prosentvise andelen er målt for kort og lang utdanning. Kort utdanning tilsvarer en batchlor, mens høy utdanning tilsvarer en master eller høyere. Figurene er satt i samme skala slik at dataen lett kan sammenlignes"

df_utdanning %>%
  filter(år>=2002) %>%
  ggplot(aes(x = år, y = vekst, fill = utdanningsnivå)) +
  geom_col(position = "dodge") +
  #geom_smooth(aes(group = 1), color = "black", size = 0.17, method = "loess") +
  facet_wrap(~utdanningsnivå, ncol = 1) +
  scale_x_continuous(breaks = seq(min(df_utdanning$år), max(df_utdanning$år), by = 2)) +
  scale_fill_manual(
    values = c("Kort utdanning" = "burlywood4", "Lang utdanning" = "darkolivegreen4") # Tilpass farger for facets
  ) +
  labs(
    ...
    y = "Vekst (%)",
    caption = "Kilde: Statistisk sentralbyrå (2024)",
    fill = "Utdanningsnivå" # Tittel på legenden
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 50, hjust = 1),
...
#| label: fig-utenskatt
#| fig-cap:
#|   - "Figuren illusterer max nytte av konsum og fritid, og deretter sammenligner lønn med arbeidledighetstrygd"

# Fikk hjelo av chatgpt med denne koden:
combined_plot_3 <- figur_1 + figur_2 +
  plot_annotation(
    title = "Optimal nytte av konsum og fridtid",
    subtitle = "Målt med budsjettbetingelse",
    theme = theme(
      plot.title = element_text(hjust = 0.5, size = 16, face = "bold"), # Midtstill tittel
      plot.subtitle = element_text(hjust = 0.5, size = 12) # Midtstill undertittel
    )
  )
...
#buds_5 <- function(F) 6000-250 * F + 2100

figur_4 <- figur_1 +
  stat_function(fun = buds_4, color = "blue") +
  #stat_function(fun = buds_5) +
...
#| label: fig-medskatt
#| fig-cap:
#|   - "Figuren viser nedgangen i lønn ved høy skatt, og deretter hvordan arbeidsledighetstrygda påvirker dette"

# Fikk hjelo av chatgpt med denne koden:
combined_plot_4 <- figur_3 + figur_4 +
  plot_annotation(
    title = "Optimal nytte av konsum og fridtid",
    subtitle = "Målt med budsjettbetingelse",
    theme = theme(
